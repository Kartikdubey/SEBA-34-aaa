...
public class AaaManager
    ...
    final List<Byte> validEapolTypes = Arrays.asList(EAPOL.EAPOL_START, EAPOL.EAPOL_LOGOFF, EAPOL.EAPOL_PACKET);
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        private void handleSupplicantPacket(InboundPacket inPacket) throws StateMachineException {
            ...
            String etherTypeHex = Integer.toHexString(ethPkt.getEtherType());
            boolean isValidLength = true;
            boolean isValidEapolType = true;
            if (len != (HEADER_LENGTH + pktlen)) {
                isValidLength = false;
                aaaStatisticsManager.getAaaStats().incrementInvalidBodyLength();
                return;
            }
            if (!validEapolTypes.contains(eapol.getEapolType())) {
                isValidEapolType = false;
                aaaStatisticsManager.getAaaStats().incrementInvalidPktType();
                return;
            }
            if (isValidLength && pktlen >= 0 && etherTypeHex.trim().equalsIgnoreCase("ffff888e")
                    && isValidEapolType) {
